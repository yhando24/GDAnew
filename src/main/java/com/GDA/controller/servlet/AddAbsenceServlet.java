...
public class AddAbsenceServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (absence.getEndDate().isAfter(absence.getStartDate())
				|| absence.getEndDate().equals(absence.getStartDate())) {
			finAfterDebut = true;
		...
		if (!Chevauchement && JourPlus1 && finAfterDebut) {
			...
			session.setAttribute("errorAdd",
					"Probleme de chevauchement de date, votre demande d'absence n'a pas été enregistrée");
			response.sendRedirect(request.getContextPath() + "/absences-management?action=addAbsence"); // logged-in
																										// page
		}
